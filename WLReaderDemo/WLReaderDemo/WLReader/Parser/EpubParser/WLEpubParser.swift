...
class WLEpubParser: NSObject, SSZipArchiveDelegate {
    ...
    func parseCoverImage(_ epubPath: String, unzipPath: String? = nil) throws -> UIImage {
        ...
        guard let image = UIImage(contentsOfFile: coverImage.fullHref) else {
            throw WLReaderError.invalidImage(path: coverImage.fullHref)
        }
    ...
    private func readContainer(with bookBasePath: String) throws {
        ...
        guard let fullPath = xmlDoc.root["rootfiles"]["rootfile"].attributes["full-path"] else {
            throw WLReaderError.fullPathEmpty
        }
        opfResource.mediaType = WLMediaType.by(fileName: fullPath)
    ...
    private func readOpf(with bookBasePath: String) throws {
        ...
        xmlDoc.root["manifest"]["item"].all?.forEach {
            ...
            resource.mediaType = WLMediaType.by(name: $0.attributes["media-type"] ?? "", fileName: resource.href)
        ...
        if let tocResource = book.resources.findByMediaType(WLMediaType.ncx) {
            book.tocResource = tocResource
        } else if let tocResource = book.resources.findByExtension(WLMediaType.ncx.defaultExtension) {
    ...
    private func readSmilFileElements(_ children: [AEXMLElement]) -> [WLSmilElement] {
        var data = [WLSmilElement]()

        // convert each smil element to a FRSmil object
        children.forEach{
            let smil = WLSmilElement(name: $0.name, attributes: $0.attributes)
    ...
    private func findTableOfContents() -> [WLTocReference] {
        var tableOfContent = [WLTocReference]()
        ...
        do {
            if tocResource.mediaType == WLMediaType.ncx {
    ...
    fileprivate func readTOCReference(_ navpointElement: AEXMLElement) -> WLTocReference? {
        var label = ""

        if book.tocResource?.mediaType == WLMediaType.ncx {
            ...
            let toc = WLTocReference(title: label, resource: resource, fragmentID: fragmentID)
    ...
    var flatTOC: [WLTocReference] {
        var tocItems = [WLTocReference]()
    ...
    func countTocChild(_ item: WLTocReference) -> [WLTocReference] {
        var tocItems = [WLTocReference]()
    ...
    fileprivate func readMetadata(_ tags: [AEXMLElement]) -> WLMetadata {
        let metadata = WLMetadata()
        ...
        for tag in tags {
            ...
            if tag.name == "dc:identifier" {
                let identifier = WLIdentifier(id: tag.attributes["id"], scheme: tag.attributes["opf:scheme"], value: tag.value)
            ...
            if tag.name == "dc:creator" {
                metadata.creators.append(WLAuthor(name: tag.value ?? "", role: tag.attributes["opf:role"] ?? "", fileAs: tag.attributes["opf:file-as"] ?? ""))
            }
            ...
            if tag.name == "dc:date" {
                metadata.dates.append(WLEventDate(date: tag.value ?? "", event: tag.attributes["opf:event"] ?? ""))
            }

            if tag.name == "meta" {
                if tag.attributes["name"] != nil {
                    metadata.metaAttributes.append(WLMeta(name: tag.attributes["name"], content: tag.attributes["content"]))
                }

                if tag.attributes["property"] != nil && tag.attributes["id"] != nil {
                    metadata.metaAttributes.append(WLMeta(id: tag.attributes["id"], property: tag.attributes["property"], value: tag.value))
                }

                if tag.attributes["property"] != nil {
                    metadata.metaAttributes.append(WLMeta(property: tag.attributes["property"], value: tag.value, refines: tag.attributes["refines"]))
                }
            }
        }
        return metadata
    ...
    fileprivate func readSpine(_ tags: [AEXMLElement]) -> WLSpine {
        let spine = WLSpine()
